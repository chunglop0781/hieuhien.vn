...
def view():
	...
	if link!=None:
		...
		if link_video.startswith(web_url):
			...
			video_src=soup.find('meta', attrs={'itemprop':'embedURL'})
			video_link=video_src.get('content')
			if 'youtube.com' in video_link:
				link_video="plugin://plugin.video.youtube/play/?video_id="+video_link.split('=')[-1]
			else:
	...
	if cat!=None:
		...
		for item in data_List:
			link_item=web_url+item.get('href').split('/')[-1]
